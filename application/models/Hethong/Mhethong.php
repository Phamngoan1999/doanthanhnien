<?php 
	...
	class Mhethong extends MY_Model
	{
		...
		public function getSV_Khoa($makhoa, $trangthai, $dieukien=null){
		...
		public function ThongKeSV_Tong(){
			$this->db->select("*");
	    	$this->db->where("tbl_sinhvien.sMaSV !=", 'admin');
			$this->db->where("tbl_khoa.makhoa	 !=", "13");
	    	$this->db->join("tbl_khoa", "tbl_sinhvien.FK_makhoa = tbl_khoa.makhoa", "inner");
	    	$this->db->order_by("tbl_khoa.tenkhoa","ASC");
	    ...
	    public function get_svKhoa($trangthai){
	    	$this->db->select("count(sMaSV) as tongsoSVDT,tbl_khoa.makhoa, tbl_khoa.tenkhoa, tbl_khoa.heso");
			// $this->db->where('tbl_sinhvien.FK_makhoa', $makhoa);
    		$this->db->where("tbl_sinhvien.trangthai_lambai", 1);
    		$this->db->where("UNIX_TIMESTAMP(tbl_sinhvien.dtTgianKetthuc)- UNIX_TIMESTAMP(tbl_sinhvien.dtTgianBatdau) <=", 1505);
	    	$this->db->join("tbl_khoa", "tbl_sinhvien.FK_makhoa = tbl_khoa.makhoa", "inner");
	    	$this->db->group_by("tbl_khoa.makhoa, `tbl_khoa`.`tenkhoa`, `tbl_khoa`.`heso`");
	    	$this->db->order_by("count(sMaSV) * tbl_khoa.heso", "DESC");
	    	$sinhvien = $this->db->get("tbl_sinhvien")->result_array();
	    	 // pr($this->db->last_query());
	    	return $sinhvien;
	    }

	    public function get_ConLai_svkhoa($makhoa){
	    	$this->db->select("count(sMaSV) as tongso");
	    	$this->db->where("FK_makhoa", $makhoa);
	    	$sinhvien = $this->db->get("tbl_sinhvien")->row_array();
	    	return $sinhvien;
	    }
	}
 ?>